...
| `downloadSettings.rssCacheRoot` | `WL_DOWNLOAD_SETTINGS__RSS_CACHE_ROOT` | `<download root>/.wireloft-rss-cache` | Root for media cached while fulfilling RSS requests. When omitted, its default follows the download root; an explicit value may point to mounted or container-local storage. |
