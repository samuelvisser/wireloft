...
def test_feed_limit_keeps_newest_items(monkeypatch):
    ...
    settings = SimpleNamespace(rss=SimpleNamespace(max_items=2))
    monkeypatch.setattr(
        "backend.api.endpoints.feeds.service.get_settings",
        lambda: settings,
    )

    items = get_feed_items(_FakeSession(episodes), _profile())

    assert [episode.id for episode, _ in items] == [0, 1]


def test_zero_feed_limit_keeps_full_history(monkeypatch):
    from backend.api.endpoints.feeds.service import get_feed_items

    now = datetime(2026, 1, 1, 12, 0, 0)
    episodes = [_episode(index, now - timedelta(days=index)) for index in range(5)]
    settings = SimpleNamespace(rss=SimpleNamespace(max_items=0))
    monkeypatch.setattr(
        "backend.api.endpoints.feeds.service.get_settings",
        lambda: settings,
    )
