...
def test_scheduler_pauses_after_database_corruption(monkeypatch, caplog) -> None:
    ...
    pauses: list[tuple[str, str | None]] = []

    def pause_scheduled_work(reason: str, *, owner_key: str | None = None):
        pauses.append((reason, owner_key))

    monkeypatch.setattr(scheduler_module, "pause_scheduled_work", pause_scheduled_work)
    ...
    assert pauses == [("SQLite database corruption", "sqlite-database-corruption")]
