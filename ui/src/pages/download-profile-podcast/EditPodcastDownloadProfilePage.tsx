...
export default function EditPodcastDownloadProfilePage() {
  ...
  const form = useForm<PodcastDownloadProfileUpdateIn>({
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const r = await fetch(`${base}/podcast-download-profiles/${profileId}`, { credentials: 'include' })
  ...
  const submitFn = async (data: PodcastDownloadProfileUpdateIn) => {
    if (!profileId) return undefined as any
    const base = (window as any).appConfig.API_URL
    return fetch(`${base}/podcast-download-profiles/${profileId}` , {
      method: 'PUT',
  ...
  const onSubmit = buildServerAwareSubmit(form as any, async (dataOut: PodcastDownloadProfileUpdateIn) => {
    ...
    return (
      <section className="view" aria-labelledby="edit-download-profile-title">
        <div className="view-header">
          <h1 id="edit-download-profile-title">Edit podcast download profile</h1>
        </div>
      ...
      {loading ? (
        ...
        <form className="form" onSubmit={onSubmit} noValidate>
          <div className="form-row">
            ...
            <div style={{ padding: '6px 0' }}>Podcast</div>
          </div>
