...
import { useDownloadProfilesView, useEpisodes, useLocalMediaProfiles, useMediaDownloadsView, useShow, useShowSeasons, useStreamProfilesView } from '../../lib/queries'
...
export default function ShowPage() {
  ...
  const {
    data: localMediaProfiles,
    isLoading: localMediaProfilesLoading,
    error: localMediaProfilesError,
  ...
  const showDownloadLocalMediaProfiles = useMemo(() => {
    const profileIds = new Set(
      (downloads ?? [])
        .filter((download) => download.showSlug === id)
        .map((download) => download.localMediaProfileId),
    )
    return (localMediaProfiles ?? []).filter((profile) => (
      profile.type === 'show' && profileIds.has(profile.id)
    ))
  }, [downloads, id, localMediaProfiles])
    ...
    : metadataRefreshOperation
      ? metadataRefreshOperation.status === 'WAITING'
        ...
        : `A metadata refresh is running for ${show.title}.${metadataRefreshOperation.progressTotal > 0
          ...
          : ''}`
      : undefined
  const localMediaProfileScopeUnknown = (
    (downloadsLoading && downloads === undefined)
    || (localMediaProfilesLoading && localMediaProfiles === undefined)
  )
  const localMediaProfileScopeFailed = (
    (Boolean(downloadsError) && downloads === undefined)
    || (Boolean(localMediaProfilesError) && localMediaProfiles === undefined)
  )
    ...
    : fileRenameOperation
      ? fileRenameOperation.status === 'WAITING'
        ...
        : `File Rename is running for ${show.title}.${fileRenameOperation.progressTotal > 0
          ...
          : ''}`
      : localMediaProfileScopeUnknown
        ? 'WireLoft is still checking which Local Media Profiles have downloads for this show.'
        : localMediaProfileScopeFailed
          ? 'WireLoft could not determine which Local Media Profiles have downloads for this show.'
          : showDownloadLocalMediaProfiles.length === 0
            ? `${show.title} has no episode downloads to rename.`
            : undefined
    ...
    : redownloadOperation
      ...
      : localMediaProfileScopeUnknown
        ...
        : localMediaProfileScopeFailed
          ? 'WireLoft could not determine which Local Media Profiles have downloads for this show.'
          : showDownloadLocalMediaProfiles.length === 0
            ? `${show.title} has no episode downloads to re-download.`
            : undefined
  ...
  const openFileRenameConfirm = () => {
    if (localMediaProfileScopeUnknown) {
      toast('WireLoft is still checking the Local Media Profiles used by this show')
      return
    }
    if (localMediaProfileScopeFailed) {
      toast.error('Could not determine which Local Media Profiles are used by this show')
      return
    }
    if (!showDownloadLocalMediaProfiles.length) {
      toast(`${show.title} has no episode downloads to rename`)
      return
    }
    setFileRenameLocalMediaProfileId(
      showDownloadLocalMediaProfiles.length > 1 ? 'all' : String(showDownloadLocalMediaProfiles[0].id),
    )
  ...
  const renameFiles = async () => {
    ...
    try {
      ...
      const profileCount = typeof result?.localMediaProfilesQueued === 'number'
        ...
        : (fileRenameLocalMediaProfileId === 'all' ? showDownloadLocalMediaProfiles.length : 1)
  ...
  const openRedownloadConfirm = () => {
    ...
    if (!showDownloadLocalMediaProfiles.length) {
      toast(`${show.title} has no episode downloads to re-download`)
      return
    }
  ...
  const redownloadAllEpisodes = async () => {
    ...
    try {
      ...
      const profileCount = typeof result?.localMediaProfilesQueued === 'number'
        ? result.localMediaProfilesQueued
        : (redownloadLocalMediaProfileId === 'all' ? showDownloadLocalMediaProfiles.length : 1)
    ...
    <section className="view show-view" aria-labelledby="show-title">
        ...
        <div className="modal-overlay" role="presentation" onClick={() => {
          ...
          >
            <div className="modal-header">
              <div className="modal-icon" aria-hidden><FontAwesomeIcon icon={['fas', 'arrows-rotate']} /></div>
            ...
            <div className="modal-actions">
              <button type="button" className="btn" disabled={metadataRefreshBusy} onClick={() => setMetadataRefreshConfirm(false)}>Cancel</button>
              <button type="button" className="btn btn-primary" disabled={metadataRefreshBusy} onClick={() => void refreshAllMetadata()}>
      ...
      {fileRenameConfirm && (
        <div className="modal-overlay" role="presentation" onClick={() => {
          ...
          >
            <div className="modal-header">
              <div className="modal-icon" aria-hidden><FontAwesomeIcon icon={['fas', 'pen-to-square']} /></div>
              <h2 id="file-rename-title" className="modal-title">File Rename</h2>
            </div>
            <p id="file-rename-desc" className="modal-text">
              Rename existing episode files in "{show.title}" so their paths match the current Local Media Profile output templates. Choose which Local Media Profile to apply.
            </p>
            <div className="form-row">
              ...
              >
                {showDownloadLocalMediaProfiles.length > 1 && (
                  <option value="all">All Local Media Profiles</option>
                )}
                {showDownloadLocalMediaProfiles.map((profile) => (
            ...
            <div className="modal-actions">
              <button type="button" className="btn" disabled={fileRenameBusy} onClick={() => setFileRenameConfirm(false)}>Cancel</button>
              <button type="button" className="btn btn-primary" disabled={fileRenameBusy || !fileRenameLocalMediaProfileId} onClick={() => void renameFiles()}>
      ...
      {redownloadConfirm && (
        <div className="modal-overlay" role="presentation" onClick={() => {
          ...
          >
            <div className="modal-header">
              <div className="modal-icon danger" aria-hidden><FontAwesomeIcon icon={['fas', 'arrows-rotate']} /></div>
            ...
            <div className="modal-actions">
              <button type="button" className="btn" disabled={redownloadBusy} onClick={() => setRedownloadConfirm(false)}>Cancel</button>
              <button type="button" className="btn btn-danger" disabled={redownloadBusy || !redownloadLocalMediaProfileId} onClick={() => void redownloadAllEpisodes()}>
      ...
      {confirm && (
        <div className="modal-overlay" role="presentation" onClick={closeConfirm}>
          ...
          >
            <div className="modal-header">
              <div className="modal-icon danger" aria-hidden><FontAwesomeIcon icon={['fas', 'trash']} /></div>
