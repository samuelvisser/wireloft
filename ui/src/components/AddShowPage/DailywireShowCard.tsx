...
export default function DailywireShowCard({showSlug, membershipPlan}: Props) {
    const q: UseQueryResult<DailywireShowRead> = useDailywireShow(showSlug, membershipPlan)
    ...
    if (q.isError) {
        const err = q.error
        const msg = (err.message) || 'Failed to load show'
    ...
    const data: DailywireShowRead = q.data
    if (!data) return null

    const author: string = data.authorName
    const portraitUrl: string | undefined = data.thumbnailPortraitPath ?? undefined
    const bgUrl: string | undefined = data.backgroundImagePath || portraitUrl
