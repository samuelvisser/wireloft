...
export default function GeneralSettingsTab({draft, updateDraft, environmentVariableFor}: SettingsTabProps) {
    return (
        <>
            ...
            <SettingsSection
                title="RSS feeds"
                description="Controls the size of RSS feeds generated by WireLoft."
            >
                <NumberField
                    id="settings-rss-max-items"
                    label="Maximum episodes"
                    value={draft.rss.maxItems}
                    min={0}
                    environmentVariable={environmentVariableFor('rss.maxItems')}
                    onChange={(value) => updateDraft((next) => {
                        next.rss.maxItems = value
                    })}
                    help="Only the newest episodes are included. Set to 0 to expose the complete history."
                />
            </SettingsSection>
        </>
    )
