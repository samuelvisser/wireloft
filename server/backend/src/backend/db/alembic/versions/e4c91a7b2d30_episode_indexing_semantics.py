...
def _is_trailer(record) -> bool:
    if record.is_trailer is not None:
        return bool(record.is_trailer)
    return bool(_OFFICIAL_TRAILER_RE.search(record.title or ""))
...
def _resolve_possible_trailer(
    ...
    *,
    ...
    if record.is_trailer is not None:
        return record
    if not (
        _TRAILER_RE.search(record.title or "")
        or current_identifier.startswith("trailer.")
    ):
        return record
    try:
        ...
        for local_season in seasons_by_show.get(show_id, []):
            ...
            for local_episode in episodes_by_season.get(season_id, []):
                ...
                record = _resolve_possible_trailer(
                    ...
                    record,
                    require_member_exclusive=require_member_exclusive,
                    current_identifier=str(local_episode["episode_identifier"]),
                )
