...
if TYPE_CHECKING:
    from backend.db.models import DownloadProfileBase, Episode, Movie, MovieExtra
...
SHOW_OUTPUT_TEMPLATE_METADATA_SCOPES: frozenset[CustomMetadataScope] = frozenset({"show", "download"})
...
def episode_output_template_values(
    episode: "Episode",
    download_profile: "DownloadProfileBase | None" = None,
) -> dict[str, str]:
    ...
    values.update(custom_metadata_template_values(
        "download",
        getattr(download_profile, "custom_metadata", None),
    ))
    return values
...
def resolve_episode_output_path(
    ...
    *,
    ...
    rendered = render_output_template(
        output_template,
        episode_output_template_values(episode, download_profile),
