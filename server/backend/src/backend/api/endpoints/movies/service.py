...
def get_movies_list() -> list[MovieAPIRead]:
    with db_session() as s:
        ...
        return [MovieAPIRead.model_validate(it, from_attributes=True) for it in items]
...
def get_movie(movie_slug: str) -> MovieAPIRead:
    with db_session() as s:
        ...
        return MovieAPIRead.model_validate(item, from_attributes=True)
...
def delete_movie(movie_slug: str) -> MovieAPIRead:
    with db_session() as s:
        ...
        payload = MovieAPIRead.model_validate(item, from_attributes=True)
        s.delete(item)
        s.commit()
        return payload
