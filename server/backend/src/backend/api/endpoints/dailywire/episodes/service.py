...
def get_episodes_from_show_list(
    ...
    *,
    ...
    show_payload = client.get_show_page(slug=show_slug, membership_plan=membership_plan)

    show_record = ShowRecord.model_validate(show_payload)
...
def get_episodes_from_season_list(
    ...
    if not has_next:
        return [EpisodeRecord.model_validate(episode) for episode in items]
    while has_next:
        episode_list.extend([EpisodeRecord.model_validate(episode) for episode in items])
